      ...
      <Stack className={'header-box'}>
        ...
        <Stack className={'search-box'}>
          <Stack className={'select-box'}>
            <Box
              component={'div'}
              className={`box ${openBrand ? 'on' : ''}`}
              onClick={brandStateChangeHandler}
            >
              <span>
                {searchFilter?.search?.brandList ? searchFilter?.search?.brandList[0] : 'Car Make'}{' '}
              </span>
              <ExpandMoreIcon />
            </Box>
            ...
            <Box className={`box ${openFuelType ? 'on' : ''}`} onClick={fuelStateChangeHandler}>
              <span>
                {searchFilter?.search?.fuelTypeList
                  ? `${searchFilter?.search?.fuelTypeList[0]}}`
                  : 'Fuel Type'}
              </span>
              <ExpandMoreIcon />
            </Box>
          ...
          <Stack className={'search-logo'}>
            {[
              ...
              <Box
                key={item.name}
                className={'logo'}
                onClick={() => carTypeQuickSelectHandler(item.name)}
              >
