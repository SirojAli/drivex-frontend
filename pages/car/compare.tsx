...
const CompareCarPage = ({ car1, car2 }: CompareCarPageProps) => {
  ...
  const cars = [car1, car2].filter(Boolean);
  ...
  return (
    <Box sx={{ p: 1, maxWidth: '1200px', mx: 'auto', fontFamily: 'Poppins, sans-serif' }}>
      ...
      <Box display="flex" gap={4} mb={4} justifyContent="center">
        {cars
          .filter(Boolean) // remove undefined cars
          .map((car) => (
            <Box
              key={car._id}
              width={340}
            ...
            >
              ...
              <Box
                ...
                overflow="hidden"
                sx={{ cursor: 'pointer' }}
                onClick={() => car?._id && pushDetailHandler(car._id)}
              >
                <img
                  src={`${REACT_APP_API_URL}/${car?.carImages?.[0] ?? 'default-car.png'}`}
              ...
              <Box p={2}>
                ...
                <Typography variant="h5" color="#FF7101" fontWeight={600} sx={{ mt: 1 }}>
                  ₩ {car?.carPrice ? Number(car.carPrice).toLocaleString('ko-KR') : '-'}
                </Typography>
              </Box>
            </Box>
          ))}
