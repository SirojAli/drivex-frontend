...
const CarDetail: NextPage = ({ initialComment, ...props }: any) => {
  ...
  const features = [
    ...baseFeatures,
    ...(car?.carType ? carTypeFeaturesMap[car.carType] || [] : []),
  ];
                      ...
                      <Box className="avatar-name-wrapper">
                        ...
                        <Box className="avatar-name">
                          <Typography className="user-name">
                            {r.memberData?.memberFullName ?? 'User'}
                          </Typography>
                          <Box className="stars">
                            {Array(5)
                              .fill(0)
              ...
              <Stack className={'right-box'}>
                ...
                <Stack className={'seller-contact'}>
                  <Stack className={'main'}>
                    <Box
                      className={'info'}
                      onClick={() => pushBrandPageHandler(car?.memberData?.brandSlug)}
                    >
                  ...
                  <Box className="listing">
                    {popularCars.map((car: Car) => (
                      <Box
                        className="car-box"
                        key={car._id}
                        onClick={() => router.push(`/car/detail?id=${car._id}`)}
                      >
                        <img
                          src={`${REACT_APP_API_URL}/${
                            car?.carImages?.[0] ?? 'img/default-car.png'
                          }`}
                          alt={car?.carModel}
                          loading="lazy"
                        />
